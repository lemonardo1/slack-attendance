...
- 보드/리스트 뷰 전환
- 리스트 뷰 상태/담당자 필터 + 정렬
- 리스트 행 드래그 앤 드롭으로 부모-자식(서브태스크) 구조 변경
- 다중 선택 일괄 삭제
- Task Context(프로젝트 핵심 배경) 저장 후 AI 기능에 재사용
- AI Spark Subtasks 생성
- New Task 모달에서 `Enter` 즉시 생성 (`Shift+Enter` 줄바꿈)

### 🎨 웹 대시보드
- **인터랙티브 티켓 보드**: https://slack-attendance.lemonaatree.workers.dev/
  - 인라인 편집, 상태 변경, 담당자 지정
  - 리스트 필터/정렬, 계층형 업무 구성, 대량 삭제
  - AI 기반 서브태스크 분해

- **주간 통계**: https://slack-attendance.lemonaatree.workers.dev/stats
  - 로그인 보호 (Google OAuth 지원)
  - 팀원별 출근/퇴근 시간
  - 근무시간 자동 계산
  - 이전/다음 주 탐색
...
기능:
- ✅ 새 티켓 생성
- ⌨️ New Task 모달에서 `Enter`로 바로 생성 (`Shift+Enter` 줄바꿈)
- ✏️ 업무명 클릭하여 수정
- 🔄 상태 드롭다운 (대기/진행/완료)
- 👤 담당자/작성자 검색 및 선택
- 🧭 보드/리스트 뷰 전환
- 🧩 리스트 뷰에서 상태/담당자 필터 + 정렬
- 🌳 리스트 행 드래그로 부모-자식(서브태스크) 구조 변경
- 🗑️ 다중 선택 일괄 삭제
- 🤖 AI Spark Subtasks 생성 + 프로젝트 컨텍스트 연동
...
- 1-1) (선택) 회의 제목을 입력하고 `AI로 일정 생성하기` 버튼으로 권장 시간대를 자동 생성 (로그인 필요)
- 2) 생성된 링크를 팀에 공유
